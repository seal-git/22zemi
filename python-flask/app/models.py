...
def http_popular_detail():
    ...
    for user_id, current_user in current_group[group_id]['Users'].items():
        for restaurant_id, feeling in current_user['Feeling'].items():
            if restaurant_id not in popular_json:
                popular_json[restaurant_id] = {'Keep': [], 'All': []}
            if feeling:
                popular_json[restaurant_id]['Keep'].append(user_id)
            popular_json[restaurant_id]['All'].append(user_id)
    return json.dumps(popular_json)
