...
class Restaurant(Base):
    ...
    genre_code = Column('genre_code', String(
        200))  # ジャンルコード: Yahoo Local Search API を参照(\n区切り)
...
class Belong(Base):
    ...
    request_count = Column('request_count', Integer, nullable=False,
                           server_default='0')  # レコメンド # リクエスト回数
    request_restaurants_num = Column('request_restaurants_num', Integer,
                                     nullable=False,
...
class History(Base):
    ...
    feeling = Column('feeling',
                     Boolean)  # None: 表示したが未回答, True: いいね, False: 悪いね
    created_at = Column('created_at', Timestamp,
                        server_default=current_timestamp(), nullable=False)
    updated_at = Column('update_at', Timestamp, server_default=text(
        'CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'), nullable=False)
...
class Vote(Base):
    ...
    group = Column('group', Integer, primary_key=True)
    restaurant = Column('restaurant', String(50), primary_key=True)
    votes_like = Column('votes_like',
                        Integer)  # -1: 未送信 # session.query(History).filter(History.group==group_id, History.restaurant==restaurants_info[i]['Restaurant_id'], History.feeling==True).count() # レストランのいいね数
    votes_all = Column('votes_all',
                       Integer)  # -1: 未送信 # session.query(History).filter(History.group==group_id, History.restaurant==restaurants_info[i]['Restaurant_id'], History.feeling is not None).count() # レストランの投票人数
