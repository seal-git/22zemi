...
def _test_yahoo_local_search_with_param():
    params = Params()
    params.query = ""
...
def _test_yahoo_local_search_with_param_2():
    params = Params()
    params.query = "和食"
...
def _test_yahoo_local_search_with_id():
...
def _test_yahoo_contents_geocoder():
...
def _test_hotpepper_search():
    ...
    params.max_price = 2000
    # params.image = True
    restaurants_info = hotpepper_search(params=params)
    print("hotpepper_search")
    [pprint.PrettyPrinter(indent=2).pprint(r.get_dict()) for r in restaurants_info]
    assert len(restaurants_info)>0
